jQuery(document).ready(function($) {		
	...
	$('#callback-popup').on('show.bs.modal', function (e) {
		...
		$("#myModalLabel").html("<span class='glyphicon glyphicon-phone'></span> Закажите обратный звонок!");
		...
		if($(e.relatedTarget).data('product') != '' && $(e.relatedTarget).data('product') != undefined)
			$("#callback-product").val($(e.relatedTarget).data('product'));
		else if ($('.block_choose button').length > 0)
			$("#callback-product").val($('.block_choose button:first').data('product'));
		else
			$("#callback-product").val($('h1:first').text());
		if($(e.relatedTarget).data('utm_campaign') != '' && $(e.relatedTarget).data('utm_campaign') != undefined)
		...
		if($(e.relatedTarget).data('title') != '' && $(e.relatedTarget).data('title') != undefined) {
			if (fid > 0)
				$("#myModalLabel").html("<span class='glyphicon glyphicon-user'></span> "+$(e.relatedTarget).data('title'));
			else
				$("#myModalLabel").html("<span class='glyphicon glyphicon-user'></span> Нужна консультация? ("+$(e.relatedTarget).data('title')+")");

			switch (fid) {
				case 296:
					try{yaCounter213614.reachGoal('otkr-zakaz');}catch(e){}
				break;
				case 297:
					try{yaCounter213614.reachGoal('otkr-vopr');}catch(e){}
				break;
				case 521:
					try{yaCounter213614.reachGoal('otkr-demo');}catch(e){}
				break;
			}
		} else {
			try{yaCounter213614.reachGoal('OPEN_POPUP');}catch(e){}
		}
	...
	$('#callback-form').submit( function () {
		...
		switch (fid) {
			case '296':
				try{yaCounter213614.reachGoal('zakazat');}catch(e){}
			break;
			case '297':
				try{yaCounter213614.reachGoal('vopros');}catch(e){}
			break;
			case '521':
				try{yaCounter213614.reachGoal('zakaz_dem');}catch(e){}
			break;
		}

		$.post("/local/templates/index_new/save.php", $(this).serialize(), function (ret) {
			$("#submit-ret").html(ret);

			if (ret.match(/red/)) {
				$("#ConfirmLabel").html("<span class='glyphicon glyphicon-remove-circle'></span> Заявка НЕ отправлена!");
				$("#thx").hide();
			} else {
			...
			try{yaCounter213614.reachGoal('SEND_POPUP');}catch(e){}
		});
		return false;
	});
	...
	$('#task-form').submit( function () {
		...
		$(this).append('<input type="hidden" name="source" value="'+ $('h1:first').text() +'" />');
		$(this).append('<input type="hidden" name="utm_campaign" value="'+ $('.block_choose button:first').data('utm_campaign') +'" />');
...
//		return false;
	...
	$('.slide_labels').on('click', function (event) {
		...
		$('.main-slider').removeClass('active-slide');
		$('.main-slider[data-slide='+$('#slider').data("curslide")+']').addClass('active-slide');
	...
	$('.efarea').on('click', function(event) {
		...
		$('.efarea_solution[data-id='+$(this).data('id')+']').addClass('chshown');
	...
	$('.efprocess').on('click', function(event) {
		...
		$('.efprocess_solution[data-id='+$(this).data('id')+']').addClass('chshown');
	...
	$(".office-select").on('click', function(event) {
		...
		$url = $(this).data('url');
		$.get(
			"/?set_region="+encodeURIComponent($(this).data('phone')) + "&change_region=" + $(this).data('region'),
			function(ans) {
	...
	if ($('#top_region').data('cur') != undefined && $('#top_region').data('cur').length == 0) {
	var obj = new Object();
	obj.ip=ip;
	...
	$('a').each ( function () {
		if ($(this).attr('href') != '' && $(this).attr('href') != undefined  && $(this).attr('href').match(/efsol.ru/))
			$(this).attr('href',$(this).attr('href').replace(/.+efsol.ru/, ''));
	});
	...
	if (location.href.match(/\/services\//)) $('#services_link').addClass('tactive');
});
