...
if (window.isIdaStatsLoaded) {
   ...
   throw new Error('eluminate.js: Another version of the SDK has been loaded.');
}
...
/* Object for Product Usage Init Data */
...
window.idaPageIsSPA = _appInfo.idaPageIsSPA || window.idaPageIsSPA || false;

/*
 * 2017.07.30 - jleon: Identify the name of the library for control purposes 
 * 0: ida_sdk.js, 1:ida_stats.js, 2: ida_production.js, 3: eluminate.js, 4:ida_softlayer.js, -1: Unknown
 */
_appInfo.dbdmLibrary = "3";
...
(function() {
   ...
   function delegateQueue() {
      if (!!dl && !!dl.log) 
         dl.log('+++DBDM-LOG > Executing ' + window.ghostQueue.length + ' queued commands from ghostQueue.');
...
ibmweb.eluminate = {
      ...
      init: function() {
         ...
         (function(a,b,c,d) {
            ...
            var site_id = ""; 
            var loadUtagFlag = true; 

            /* 
             * utag.js will be excluded from below url pattrens and site IDs
             * use exclude_urls[] to define url pattrens, eg: exclude_urls = ["bluemix","/signup/registration"]
             * use exclude_site_ids[] to define site IDs, eg: exclude_site_ids = ["siteid1","siteid2"]
             */
            var exclude_urls = ["testpage"];
            var exclude_site_ids = [];
            ...
            /* check whether any exclude_urls[] value matches pattren with document.location.href */
            if (typeof (exclude_urls) !== 'undefined') {
               for (var i = 0; i < exclude_urls.length; i++) {
                  if (document.location.href.indexOf(exclude_urls[i]) != -1) {               
                     loadUtagFlag = false;
                     break;
                  }
               }
            ...
            /* check whether any exclude_site_ids[] value equals with site_id */
            if (loadUtagFlag && (site_id !== "") && (typeof (exclude_site_ids) !== 'undefined')) {
               for (var i = 0; i < exclude_site_ids.length; i++) {
                  if (site_id.toLowerCase() === exclude_site_ids[i].toLowerCase()) {               
                     loadUtagFlag = false;
                     break;
                  }
               }
