var _om_gbls = {
	...
	patharr : "", rsidprefix : "mssto", apl : "", split : "", addrsid : "", centrsid: "", clangloc: "", langloc: "", iroot: "", msdnlibrarynode:"",
	offprodfam : "", offprod : "", offconttype : "", titleoverride : "", prodversion : "",comscoreCookie:"",comscoreJSON:"",blogname:"",
	appliesTech : "", appliesVersion : "", w8center : "", w8section : "", startpage : "", mslocale : "",
...
scriptcenter : "tnctscript", scvmm : "tnctscvmm", sharepoint : "tnctshare", updatemanagement : "tnctupdmg", video: "tnctedge",
...
directx :"ctdx",dynamics :"ctdynamics",windowsembedded :"ctembed",evalcenter :"cteval",exchange :"ctexch", expression : "expall", fsharp :"ctfsharp",
...
var _om_msdn_split_params = {
 ...
 msdnlib_dotnet_appfabric : "libappfab",  msdnlib_dotnet_wcf : "libwcf", msdnlib_dotnet_wf : "libwf", msdnlib_pandp : "libpandp",
	msdnlib_expression : "explib"
}
// Expression Library rollup
...
try { _om_gbls.mslocale =_om_gbls.getOmniMeta("Ms.Locale").toLowerCase(); } catch(err) {}
...
if (_om_gbls.host.indexOf('msdn')>-1) { 
	...
	if (_om_gbls.path.indexOf('/dynamics/ax') > -1) _om_gbls.sa = _om_gbls.apl(_om_gbls.sa,"ctdynax",",",2);
	if (_om_gbls.path.indexOf('/library/live/') > -1) _om_gbls.sa = _om_gbls.apl(_om_gbls.sa,"ctwinlive",",",2);	
}

if (_om_gbls.host=="technet.microsoft.com" || _om_gbls.host=="msdn.microsoft.com") {
...
_om_gbls.ad('h','blogs.msdn.com','blogsall');
_om_gbls.ad('h','blogs.technet.com','blogsall');
...
if (_om_gbls.host=="help.outlook.com") {
	...
	try {_om_gbls.market=_om_gbls.mslocale.toLowerCase(); } catch(err) {}
}
...
switch (_om_gbls.host)
{
	case "msdn.microsoft.com":
		...
		if (_om_gbls.path.indexOf("/library")>-1) {  // updated 12/7/12
...
try { 
	var cllmeta = _om_gbls.getOmniMeta("ContentLanguage").toLowerCase();
	var cllpos =  cllmeta.indexOf("l="); 
	_om_gbls.clangloc = cllmeta.substring(cllpos+2,cllpos+7);
} catch(e) {}
...
if ( _om_gbls.prodversion) { // from DCS.appliesToProduct
	...
	} else if (tvar=="windowsazure") { _om_gbls.sa =_om_gbls.apl(_om_gbls.sa,"ctazure",",",2); // msdn center windows azure, added 3/23/2013
	} else if (tvar=="windowsserver_dev") { _om_gbls.sa =_om_gbls.apl(_om_gbls.sa,"libwsrvdv",",",2); // MSDN Library Windows Server, added 3/23/2013
	} else if (tvar=="windowspowershell_dev") { _om_gbls.sa =_om_gbls.apl(_om_gbls.sa,"libpowshell",",",2); // MSDN Library Windows PowerShell, added 3/23/2013
	} else if (tvar=="windowsazure_dev") { _om_gbls.sa =_om_gbls.apl(_om_gbls.sa,"libazurdv",",",2); // MSDN Library Windows Azure, added 3/23/2013
	} else if (tvar=="forefront_dev") { _om_gbls.sa =_om_gbls.apl(_om_gbls.sa,"libffrnt",",",2); // MSDN Library Forefront, added 3/23/2013
	...
	} else if (tvar=="windows server 2012") { _om_gbls.sa =_om_gbls.apl(_om_gbls.sa,"tnlibwinsrv12",",",2); 
	} 				
}
...
if (_om_gbls.host == "msdn.microsoft.com") {
	if(_om_gbls.path.indexOf('/office/apps') > -1  && _om_gbls.path.indexOf('library') < 0 ) {
...
//Dynamics AX2012 rollup:   _technet_dynamics_ax12:"tnlibdynaxad12" ||  msdnlib_es_dev_ax2012:"libdynax12",
if (_om_gbls.sa.indexOf("tnlibix")>-1 || _om_gbls.sa.indexOf("libdynax12")>-1) {
	_om_gbls.sa =_om_gbls.apl(_om_gbls.sa,"dynax12roll",",",2);
}
...
if (_om_gbls.market=="library") _om_gbls.market = _om_gbls.mslocale;
...
if (s.prop8 && s.prop13) {
	if (s.prop8.indexOf(s.prop13)>-1) {
		// if shortid is in the iroot location in pathname, then don't treat as center
		s.prop8 = ""; 
	}
}
...
if (_om_gbls.w8center) {
	...
	if (s.prop5) {
		_om_gbls.appendCustomMeta("ms.Center",s.prop5);
	}
	if (s.prop6) {
		_om_gbls.appendCustomMeta("ms.Section",s.prop6);	
	}
	
	if (s.prop13 && s.prop6) { 
		_om_gbls.appendCustomMeta("ms.ShortId",s.prop13);	
		_om_gbls.appendCustomMeta("ms.TOC",s.prop6 + ": " + s.prop13);
	} else if (s.prop6) {
		_om_gbls.appendCustomMeta("ms.TOC",s.prop6);
	}
}
...
if (_om_gbls.clangloc) s.prop18=_om_gbls.clangloc; 
...
// forums new url structure
if (_om_gbls.host.indexOf("social")>-1 && _om_gbls.path.indexOf("forums")>-1) {
	s.prop10 = "D=g";
	s.prop21 = _om_gbls.getOmniMeta('Search.MSForums.Iroot');
	s.prop22 = _om_gbls.getOmniMeta('Search.MSForums.CategoryName');
	s.prop23 = s.getQueryParam('forum'); // forum name
	s.prop24 = s.getQueryParam('category'); // categories list var	
}
...
try {
	if (_om_gbls.path.indexOf("dev10.query")>-1) {
		s.eVar49="dev10.query";
	} else if (_om_gbls.path.indexOf("dev11.query")>-1) {
		s.eVar49="dev11.query";
	} else if (_om_gbls.path.indexOf("d=hv.2")>-1) {
		s.eVar49="Help Viewer hv.2";
	} 
